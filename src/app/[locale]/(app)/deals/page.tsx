...
export default async function DealsPage() {
  ...
  if (schema === "geg_singapore") {
    ...
    const [{ data: guDeals }, { data: guAccounts }] = await Promise.all([
      db.from("deals").select("*, deal_date, country, original_request, contact_name, contact_role, contact_email, contact_phone, sector, preferred_channel, selling_price, cost, account:accounts(id, name, type)").order("created_at", { ascending: false }),
